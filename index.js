...
async function fetchUserWeatherInfo(coordinates) {
  ...
  try {
    ...
    loadingScreen.classList.remove("active");
    displayErrorMessage("Error fetching weather data. Check your internet connection.");
  }
}
...
function renderWeatherInfo(data) {
  ...
  tempToggle.addEventListener("change", () => {
    if (tempToggle.checked) {
      ...
      feelLike.innerText = `Feels Like : ${((data?.main?.feels_like * 9) / 5 + 32).toFixed(2)} °F`;
    } else {
      temp.innerText = `${data?.main?.temp} °C`;
      feelLike.innerText = `Feels Like : ${data?.main?.feels_like} °C`;
    }
...
function getLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(showPosition, () => {
      displayErrorMessage("Location access denied. Please enable location services.");
    });
  } else {
    alert("Live location is not supported on your device.");
  }
}
